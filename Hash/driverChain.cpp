/*H**********************************************************************
* FILENAME :        driverChain.cpp
*
* DESCRIPTION :
*       File tests HashTable class with chaning
*
* NOTES :
* 
...
void printUsage()
{
    cerr << "USAGE: ./driverChain.out [Data Set: A or B]" << endl;
}
 ...
 * --------------------
 *  Inserts data from an array into a hash table using chaning
 * 
...
void generateCSV(double insert[], double search[], int count, string setName)
{
    ...
    if (setName == "A")
    {
        outputFile.open("Outputs/insert_search_performance_hash_table_chain_dataSetA.csv");
    }
    else if (setName == "B")
    {
        outputFile.open("Outputs/insert_search_performance_hash_table_chain_dataSetB.csv");
    }
    if (outputFile.is_open())
    {
        ...
        if (setName == "A")
        {
            cout << "insert_search_performance_hash_table_chain_dataSetA.csv" << endl;
        }
        else if (setName == "B")
        {
            cout << "insert_search_performance_hash_table_chain_dataSetB.csv" << endl;
        }
    ...
    if (argc == 2) // Correct number of command line args
    {
        ...
        if (setName == "A" || setName == "B")
        {
            ...
            for (int numEntries = 0; numEntries < MAX_ELEMENTS; numEntries += 100)
            {
                ...
                {
                    h.searchItemChain(randInts[i]); // Search for each randomly generated key
                }
