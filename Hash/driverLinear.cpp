/*H**********************************************************************
* FILENAME :        driverLinear.cpp
*
* DESCRIPTION :
*       File tests HashTable class with linear probing
*
* NOTES :
* 
...
void printUsage()
{
    cerr << "USAGE: ./driverLinear.out [Data Set: A or B]" << endl;
}
...
void generateCSV(double insert[], double search[], int count, string setName)
{
    ...
    if (setName == "A")
    {
        outputFile.open("Outputs/insert_search_performance_hash_table_linear_dataSetA.csv");
    }
    else if (setName == "B")
    {
        outputFile.open("Outputs/insert_search_performance_hash_table_linear_dataSetB.csv");
    }
    if (outputFile.is_open())
    {
        ...
        if (setName == "A")
        {
            cout << "insert_search_performance_hash_table_linear_dataSetA.csv" << endl;
        }
        else if (setName == "B")
        {
            cout << "insert_search_performance_hash_table_linear_dataSetB.csv" << endl;
        }
    ...
    if (argc == 2) // Correct number of command line args
    {
        ...
        if (setName == "A" || setName == "B")
        {
            ...
            for (int numEntries = 0; numEntries < MAX_ELEMENTS; numEntries += 100)
            {
                ...
                {
                    h.searchItemLinear(randInts[i]); // Search for each randomly generated key
                }
